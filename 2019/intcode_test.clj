...
(deftest run-test-day-2
  (is (= [2,0,0,0,99] (vals (:data (sut/run "1,0,0,0,99")))))
  (is (= [2,3,0,6,99] (vals (:data (sut/run "2,3,0,3,99")))))
  (is (= [2,4,4,5,99,9801] (vals (:data (sut/run "2,4,4,5,99,0")))))
  (is (= [30,1,1,4,2,5,6,0,99] (vals (:data (sut/run "1,1,1,4,99,5,6,0,99")))))
...
(deftest run-test-day-5-1
  (is (= [1234] (:out (sut/run "3,0,4,0,99" [1234]))))
  (is (= [1002 4 3 4 99] (vals (:data (sut/run "1002,4,3,4,33")))))
  (is (= [11002 4 3 4 99] (vals (:data (sut/run "11002,4,3,4,33")))))
  (is (= [1101 100 -1 4 99] (vals (:data (sut/run "1101,100,-1,4,0")))))
...
(deftest run-test-day-9
  (let [env (-> (sut/make-env [109 19 204 -34] [])
                (assoc :relative-base 2000)
                (assoc-in [:data 1985] :target-value))
        after-one (sut/eval-expr env (sut/read-expr env))
        after-two (sut/eval-expr after-one (sut/read-expr after-one))]
    (is (= 2019 (:relative-base after-one)))
    (is (= [:target-value] (:out after-two))))
  ...
  (is (= 16 (count (str (first (:out (sut/run "1102,34915192,34915192,7,4,7,99,0")))))))
  (is (= 1125899906842624 (first (:out (sut/run "104,1125899906842624,99")))))
  (is (= [2870072642] (:out (sut/run (slurp "day-9.txt") [1]))))
  #_(is (= [58534] (:out (sut/run (slurp "day-9.txt") [2])))))
